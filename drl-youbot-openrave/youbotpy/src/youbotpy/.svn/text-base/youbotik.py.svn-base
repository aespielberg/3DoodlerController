...
def SolveJ4(youbot,eepose,j0j1j2j3):
    with youbot:
        ...
        eepose_with_zero_j4 = youbot.GetManipulators()[0].GetEndEffectorTransform() 
        #link4tr = youbot.GetLink('link4').GetTransform()
        eerot = np.dot(np.linalg.inv(eepose_with_zero_j4[:3,:3]),eepose[:3,:3])
        j4angle = -1.0 * np.arctan2(eerot[1,0],eerot[0,0])
