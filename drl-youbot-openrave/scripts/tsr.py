...
class GraspTSR(object):
    def __init__(self,T0_w_obj,Tw_e_list,Bw_list):
        self.T0_w_obj = T0_w_obj
        self.Tw_e_list = Tw_e_list
        self.Bw_list = Bw_list

    def Sample(self,env):
        T0_w = env.GetKinBody(self.T0_w_obj).GetTransform()
        Tw_sample = self.SampleObjectRelative(env)
        sample = np.dot(T0_w,Tw_sample)
        return sample
    ...
    def UniformSamples(self,env,translational_resolution,rotational_resolution):
        samples = []
    ...
    def Visualize(self,env,robot,nsamples=100):
...
class LookAtTSR:
    def __init__(self,tsrtype,T0_w_obj,Tw_e,normal):
        self.T0_w_obj = T0_w_obj
        self.Tw_e = Tw_e
        self.normal = normal
